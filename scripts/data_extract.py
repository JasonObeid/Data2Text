...
def substitute_variables(table_list, link_list, summary_list, game_entity_list, json_data, verbose=False):
    assert len(table_list) == len(link_list) == len(summary_list) == len(game_entity_list) == len(json_data)

    new_summary_list = []

    for table, links, summary_content, game_entities, game in zip(table_list, link_list, summary_list, game_entity_list, json_data):
        if verbose:
            print(table)
            print(links)
            print(summary_content)
        ...
        #home ranges from 12/13 to 24/25, to avoid predicting incorrect teams, I replace with home[0:12] and away[0:12]
        homeRange = (0, max(home)-min(home))
        awayRange = (0, max(away))

        table_entities = [each.split('|')[0] for each in table]
        table_headers = [each.split('|')[1] for each in table]
        table_contents = [each.split('|')[2] for each in table]
        table_outcomes = [each.split('|')[3] for each in table]
        ...
        for link in links:
            ...
            #token count is always 1
            #token_count = to_idx - from_idx
            #if token_count > 1:
            #    print(new_summary[from_idx:to_idx])

            entity = table_entities[position_in_table]
            header = table_headers[position_in_table]
            value = table_contents[position_in_table]
            #outcome = table_outcomes[position_in_table]
            if entity in game_entities:
                label = game_entities[entity]
                template = getTemplate(home, away, entity, header, value, label)
            new_summary[from_idx:to_idx] = template
...
def getTemplate(home, away, entity, header, value, label):
    if label.isnumeric():
        label = int(label)
        #check if player is home or away
        if label > max(away):
            team = 0 #home
            label = label - min(home)
        else:
            team = 1 #away
        template = [f'var_player[{team}][{label}][{header}]']
    else:
        if 'home' in label:
            team = 0 #home
        elif 'vis' in label:
            team = 1 #away
        template = [f'var_team[{team}][{header}]']
    return template
...
if __name__ == '__main__':
    ...
    substituted_summary_list = substitute_variables(table_list, final_links, summary_list, game_entity_list, json_data, args.verbose)
    ...
    with open(args.output + ".summary", "w") as outf:
        for summary in substituted_summary_list:
            outf.write("{}\n".format(' '.join(summary)))
    outf.close()
